function [subTcs, subTcs_nz] = post_processing(subTcs, TR, band)
    ...
    %
    % Outputs:
    % 1. subTcs - detrended, despiked, filtered, and zscored fMRI time
    ...
    subTcs = zeros(size(subTcs, 1), size(subTcs, 2), size(subTcs, 3));
    ...
    for sub = 1:size(subTcs_nz, 1)
        fprintf('subject: %d\n', sub)
        subTcs_nz(sub, :, :) = detrend(squeeze(subTcs_nz(sub, :, :))); % Linear Detrending
        
        subTcs_nz(sub, :, :) = icatb_despike_tc(squeeze(subTcs_nz(sub, :, :)), TR); % Linear De-spiking
        
        subTcs_nz(sub, :, :) = filtering(squeeze(subTcs_nz(sub, :, :)), srate, band); % Filtering
        
        subTcs(sub, :, :) = zscore(squeeze(subTcs_nz(sub, :, :))); % Z-scoring
    end  
end
