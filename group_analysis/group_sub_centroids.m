function group_sub_clusters = group_sub_centroids(group_est, idx, cluster_num)
    ...
    %
    % INPUTS:
    %   group_est: A 4D numeric array of group estimates (subjects x time points x features x features).
    %   idx: A 2D numeric array of cluster indices (subjects x time points).
    %   cluster_num: An integer specifying the number of clusters.
    %
    % OUTPUT:
    %   group_sub_clusters: A 4D array of grouped sub-centroids (subjects x clusters x features x features).
    %
    % Example:
    ...
    if ndims(group_est) ~= 4
        error('group_est must be a 4-dimensional array.');
    end
    ...
    group_sub_clusters = zeros(sub_size, cluster_num, feat_num, feat_num); % Pre-allocate cluster centroids for subs for group1 
